...
function getQuestions() {
  ...
  sheets.spreadsheets.values.get({auth:oauth2Client,spreadsheetId:mySheetId,range:encodeURI('QNA')},function(err,response){if(err){console.log('讀取問題檔的API產生問題：' + err);return;}
...
bot.on('message', function(event) { 
   if ((event.message == 'Lunch')&&(event.message.type === 'text')){
